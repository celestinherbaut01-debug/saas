-- L'onboarding demande désormais "Que souhaitez-vous faire avec
-- ProspectFlow ?" (product_mode) et rend la sélection de cibles de
-- prospection OPTIONNELLE quand le client a choisi "Gérer mon entreprise"
-- uniquement — on ne va pas l'obliger à choisir des métiers à démarcher
-- s'il vient de dire qu'il ne veut pas prospecter. Reprend la logique de
-- vérification stricte de 0014 (insert puis vérification réelle), signature
-- changée (nouveau paramètre) donc ancienne signature explicitement
-- supprimée d'abord, comme le faisait déjà 0014 pour la même raison.

drop function if exists public.complete_onboarding(
  text, text, text, text, uuid, text, text, text, double precision, double precision, uuid[], numeric
);
...
begin
  ...
  -- Cibles de prospection obligatoires SAUF si le client a explicitement
  -- choisi de ne pas prospecter (product_mode = 'business_os').
  ...
  insert into public.business_profiles (
    ...
    own_category_id, street, postal_code, city, lat, lng, default_radius_km, product_mode
  ) values (
